...
def build_dataset(file_pattern,
  ...
  def _example_parser(serialized_example):
    ...
    for feature_name, feature in input_config.features.items():
      ...
      if feature.is_time_series and feature.ndims > 1:
        # Time series features with multiple dimensions are encoded as separate
        # single-dimensional features 'feature_0', 'feature_1', ...
        for i in range(feature.ndims):
          data_fields["{}_{}".format(feature_name, i)] = feature_spec
      else:
    ...
    for feature_name, feature in input_config.features.items():
      if feature.is_time_series:
        if feature.ndims > 1:
          values = [
              parsed_features.pop("{}_{}".format(feature_name, i))
              for i in range(feature.ndims)
          ]
